...
$score = 0;
if (preg_match('/googlebot|bingbot|slurp|duckduckbot|baiduspider|yandexbot|applebot|msnbot|facebookexternalhit|twitterbot|linkedinbot|whatsapp|telegrambot|discordbot/i', $ua)) $score += 10;
...
$dc = ['104.131.','134.209.','157.230.','159.89.','167.99.','45.33.','51.75.','35.190.','34.96.','13.','52.','54.'];
...
/* ─── LOG DE VISITAS (solo usuarios reales, para segmentación FB Ads) ─── */
if (!$is_bot) {
    // 1. Geolocalización vía ip-api.com (gratis, sin API key, hasta 45 req/min)
    $geo = ['country' => '?', 'region' => '?', 'city' => '?', 'isp' => '?'];
    if (filter_var($ip, FILTER_VALIDATE_IP) && !preg_match('/^(10\.|192\.168\.|172\.(1[6-9]|2[0-9]|3[01])\.|127\.)/', $ip)) {
        $ch = curl_init("http://ip-api.com/json/{$ip}?fields=country,regionName,city,isp,query&lang=es");
    ...
    // 2. Parser de User-Agent (OS, marca/modelo, navegador)
    $os = 'Desconocido'; $device = 'PC/Desktop'; $brand = '?'; $browser = '?';
    if (preg_match('/Android\s+([\d.]+)/i', $ua, $m))        { $os = "Android {$m[1]}"; $device = 'Móvil'; }
    elseif (preg_match('/iPhone OS ([\d_]+)/i', $ua, $m))    { $os = 'iOS ' . str_replace('_', '.', $m[1]); $device = 'iPhone'; $brand = 'Apple'; }
    elseif (preg_match('/iPad.*OS ([\d_]+)/i', $ua, $m))     { $os = 'iPadOS ' . str_replace('_', '.', $m[1]); $device = 'iPad'; $brand = 'Apple'; }
    elseif (preg_match('/Windows NT ([\d.]+)/i', $ua, $m))   { $os = "Windows NT {$m[1]}"; }
    elseif (preg_match('/Mac OS X ([\d_]+)/i', $ua, $m))     { $os = 'macOS ' . str_replace('_', '.', $m[1]); $brand = 'Apple'; }
    elseif (stripos($ua, 'Linux') !== false)                 { $os = 'Linux'; }

    // Marca / modelo (Android)
    if ($device === 'Móvil' && preg_match('/;\s*([^;)]+?)\s+Build\//', $ua, $m)) {
        $model = trim($m[1]);
        if (stripos($model, 'SM-')   === 0 || stripos($ua, 'Samsung') !== false) $brand = 'Samsung';
        elseif (preg_match('/(Mi |Redmi|POCO|Xiaomi)/i', $model))                $brand = 'Xiaomi';
        elseif (stripos($model, 'HUAWEI') !== false || stripos($ua,'HUAWEI')!==false) $brand = 'Huawei';
        elseif (stripos($model, 'Motorola') !== false || stripos($model,'Moto')!==false) $brand = 'Motorola';
        elseif (stripos($model, 'LG-') === 0)                                    $brand = 'LG';
        elseif (stripos($ua, 'OPPO') !== false)                                  $brand = 'OPPO';
        elseif (stripos($ua, 'vivo') !== false)                                  $brand = 'Vivo';
        elseif (stripos($ua, 'Realme') !== false)                                $brand = 'Realme';
        elseif (stripos($ua, 'OnePlus') !== false)                               $brand = 'OnePlus';
        elseif (stripos($ua, 'Pixel') !== false)                                 $brand = 'Google Pixel';
        elseif (stripos($ua, 'TECNO') !== false)                                 $brand = 'Tecno';
        elseif (stripos($ua, 'Infinix') !== false)                               $brand = 'Infinix';
        else                                                                     $brand = $brand === '?' ? 'Android' : $brand;
        $device = "$brand $model";
    }
    ...
    $linea    = sprintf(
        "[%s] IP: %-15s | %s / %s / %s | ISP: %s | OS: %s | Dispositivo: %s | Navegador: %s | Ref: %s\n",
        $fecha, $ip,
        $geo['country'], $geo['region'], $geo['city'],
        $geo['isp'], $os, $device, $browser, $ref
    );

    $log_dir = __DIR__ . '/logs';
    if (!is_dir($log_dir)) @mkdir($log_dir, 0755, true);
    @file_put_contents($log_dir . '/visitas.txt', $linea, FILE_APPEND | LOCK_EX);
}
